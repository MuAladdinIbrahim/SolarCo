class NotificationController < ApiController
  ...
  def updateAll
    if current_user
      ...
      @notifications = PublicActivity::Activity.order("created_at DESC").where("(recipient_type = ? AND trackable_type = ? AND recipient_id = ?) OR (owner_type = ? AND trackable_type =? AND trackable_id IN (?))", "User", "Message", current_user.id, "Contractor", "Offer", @post_offers.ids.each {|id| id.to_s + ','}).update_all(checked: true)
      ...
      @notifications = PublicActivity::Activity.order("created_at DESC").where("(recipient_type = ? AND trackable_type = ? AND recipient_id = ?) OR (owner_type = ? AND trackable_type = ? AND trackable_id IN (?))", "Contractor", "Message", current_contractor.id, "User", "Post", @offers_posts.each {|id| id.to_s + ','}).update_all(checked: true)
    end
  ...
  def getCount 
    if current_user
      ...
      @notifications = PublicActivity::Activity.order("created_at DESC").where("((recipient_type = ? AND trackable_type = ? AND recipient_id = ?) OR (owner_type = ? AND trackable_type =? AND trackable_id IN (?)))  AND checked = false ", "User", "Message", current_user.id, "Contractor", "Offer", @post_offers.ids.each {|id| id.to_s + ','}).size
      ...
      @notifications = PublicActivity::Activity.order("created_at DESC").where("((recipient_type = ? AND trackable_type = ? AND recipient_id = ?) OR (owner_type = ? AND trackable_type = ? AND trackable_id IN (?)))  AND checked = false ", "Contractor", "Message", current_contractor.id, "User", "Post", @offers_posts.each {|id| id.to_s + ','}).size
    end
    render json: {count: @notifications}
  end
